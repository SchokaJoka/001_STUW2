...
-- Create black cards table (question cards)
CREATE TABLE black_cards (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  pack_id UUID REFERENCES card_packs(id) ON DELETE CASCADE,
  text TEXT NOT NULL,
  pick INTEGER DEFAULT 1, -- how many white cards to pick
  created_at TIMESTAMP WITH TIME ZONE DEFAULT TIMEZONE('utc', NOW())
);

-- Create white cards table (answer cards)
...
CREATE INDEX idx_black_cards_pack_id ON black_cards(pack_id);
CREATE INDEX idx_white_cards_pack_id ON white_cards(pack_id);

-- Enable Row Level Security
ALTER TABLE card_packs ENABLE ROW LEVEL SECURITY;
ALTER TABLE black_cards ENABLE ROW LEVEL SECURITY;
ALTER TABLE white_cards ENABLE ROW LEVEL SECURITY;

-- Create policies (allow read access to all)
CREATE POLICY "Allow public read access on card_packs" ON card_packs
  FOR SELECT USING (true);

CREATE POLICY "Allow public read access on black_cards" ON black_cards
  FOR SELECT USING (true);

CREATE POLICY "Allow public read access on white_cards" ON white_cards
  FOR SELECT USING (true);
